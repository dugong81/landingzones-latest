locals {
  prod_service_connector_policy = {
    name        = "${var.resource_label}-OCI-ELZ-P-SC-Policy"
    description = "OCI ELZ Service Connector Policy - Prod"
    statements = [
      ...
      "Allow any-user to manage objects in compartment id ${module.prod_environment.compartment.logging.id} where all {request.principal.type='serviceconnector', any{target.bucket.name='${var.resource_label}_${var.prod_environment_prefix}_auditLogs_standard', target.bucket.name='${var.resource_label}_${var.prod_environment_prefix}_defaultLogs_standard', target.bucket.name='${var.resource_label}_${var.prod_environment_prefix}_serviceEvents_standard'}, request.principal.compartment.id='${module.prod_environment.compartment.security.id}'}",
    ]
  }

  prod_archive_log_bucket = {
  ...
  prod_key_archive_policy = {
    name        = "${var.resource_label}-OCI-ELZ-P-KEY-ARC-Policy"
    description = "OCI Enterprise Landing Zone Key Policy For Archive - Prod"
  ...
  prod_archive_audit_log_service_connector = {
    display_name  = "${var.resource_label}_schAuditLog_archive_${var.prod_environment_prefix}"
    source_kind   = "logging"
    target_kind   = "objectStorage"
    log_group_id  = "_Audit_Include_Subcompartment"
    target_bucket = module.prod_archive_bucket.bucket.name
  }

  prod_archive_default_log_service_connector = {
    display_name  = "${var.resource_label}_schDefaultLog_archive_${var.prod_environment_prefix}"
    source_kind   = "logging"
    target_kind   = "objectStorage"
    target_bucket = module.prod_archive_bucket.bucket.name
  }
  ...
  prod_archive_service_events_service_connector = {
    display_name  = "${var.resource_label}_schServiceEvents_archive_${var.prod_environment_prefix}"
  ...
  prod_archive_key = {
    name            = "${var.resource_label}-OCI-ELZ-ARC-KEY-${var.prod_environment_prefix}"
    shape_algorithm = "AES"
    shape_length    = 32
  ...
  prod_group_names = {
    prod_platform_admin_group_name : var.prod_platform_admin_group_name != "" ? var.prod_platform_admin_group_name : "OCI-ELZ-UGP-${var.prod_environment_prefix}-PLT-ADMIN",
  }

  prod_platform_admin_policy = {
    name        = "OCI-ELZ-UGP-${var.prod_environment_prefix}-PLT-ADMIN-POLICY-ARCHIVE"
    description = "OCI Landing Zone Platform Admin Group Policy To Manage Archive Bucket"

    statements = [
      "Allow group ${local.prod_group_names["prod_platform_admin_group_name"]} to manage buckets in compartment ${module.home_compartment.compartment_name}",
      "Allow group ${local.prod_group_names["prod_platform_admin_group_name"]} to manage objects in compartment ${module.home_compartment.compartment_name}"
    ]
  }

}

module "prod_service_connector_policy" {
  source           = "../../modules/policies"
  compartment_ocid = module.home_compartment.compartment_id
  policy_name      = local.prod_service_connector_policy.name
  description      = local.prod_service_connector_policy.description
  statements       = local.prod_service_connector_policy.statements

  depends_on = [module.prod_environment]
}

module "prod_archive_key" {
  source              = "../../modules/key"
  compartment_ocid    = module.prod_environment.compartment.security.id
  display_name        = local.prod_archive_key.name
  shape_algorithm     = local.prod_archive_key.shape_algorithm
  shape_length        = local.prod_archive_key.shape_length
  protection_mode     = local.prod_archive_key.protection_mode
  management_endpoint = module.prod_environment.vault_id

  depends_on = [module.prod_environment]
}

module "prod_key_archive_policy" {
  source           = "../../modules/policies"
  compartment_ocid = module.home_compartment.compartment_id
  policy_name      = local.prod_key_archive_policy.name
  description      = local.prod_key_archive_policy.description
  statements       = local.prod_key_archive_policy.statements

  depends_on = [module.prod_environment]
}

module "prod_archive_bucket" {
  source                              = "../../modules/bucket"
  tenancy_ocid                        = var.tenancy_ocid
  compartment_id                      = module.home_compartment.compartment_id
  name                                = local.prod_archive_log_bucket.name
  kms_key_id                          = module.prod_archive_key.key_ocid
  storage_tier                        = local.prod_archive_log_bucket.bucket_storage_tier
  retention_rule_display_name         = local.prod_archive_log_bucket.retention_rule_display_name
  retention_policy_duration_amount    = local.prod_archive_log_bucket.retention_policy_duration_amount
  retention_policy_duration_time_unit = local.prod_archive_log_bucket.retention_policy_duration_time_unit
  namespace                           = data.oci_objectstorage_namespace.ns.namespace

  depends_on = [module.prod_environment, module.prod_archive_key, module.prod_key_archive_policy]
}
...
module "prod_archive_audit_log_service_connector" {
  ...
  depends_on = [module.prod_archive_bucket, module.service_connector_archive_policy]
}
