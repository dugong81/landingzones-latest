...
locals {
  ...
  management_compartment = {
    name        = "OCI-ELZ-${var.environment_prefix}-SRD-MGMT"
    description = "Management and Tools Compartment"
  }
...
module "management_compartment" {
  ...
  compartment_name          = local.management_compartment.name
  compartment_description   = local.management_compartment.description
  enable_compartment_delete = var.enable_compartment_delete
